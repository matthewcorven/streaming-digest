...
**Video Health**:
The single derived, read-model badge shown for a video (e.g. the `processingStatus` on search result cards), computed as the worst of: ingestion status, any failed or deferred non-skippable stage, and any stale search document or cluster embedding. Precedence (worst first): `failed` > `unavailable` > `deferred` > `stale` > `processed_with_warnings` > `processed` — `unavailable` slots between failure and transient states: a terminal, user-relevant fact ("don't bother clicking watch") that outranks deferrals and staleness but is not a system failure. Stage-level statuses remain for run-detail and diagnostics views; they never drive the top-level badge independently. A terminal-dead Resource does not propagate to its parent video's badge: it is an artifact-level flag on the expanded card's link row ("source unavailable — archived copy searchable"), because Video Health answers "can I use this video's content?" — archived content is fully usable, and propagating internet churn would drift every old video's badge red over time.
...
**Segment Generation**:
One complete, versioned set of Segments for a video. Exactly one generation is Active per video at a time. A new generation created by explicit user action starts inactive and may require approval before activation; generations are never deleted in MVP, preserving history. A pending candidate's Search Documents and embeddings are built at generation-creation time but tagged to their generation and excluded from search until activation — approval is then a cheap atomic flip (pointer change plus screenshot/Note bookkeeping) rather than the start of a heavy embedding job that would leave the video half-migrated, and the one-video-one-cluster invariant never sees two overlapping segment sets. Documents for a rejected candidate are cleaned up by the same reprocess machinery; the wasted embeddings are accepted MVP cost since the user previews the candidate before approving. While a candidate pends, the Active generation is fully live — its segments searchable, its Screenshots served, its Notes boosting.
...
**High-Signal Match**:
A per-ingestion-run digest feature: newly ingested items whose cluster fingerprint clears the global similarity threshold (default 80%) against a Recent Search embedding, measured in raw cosine similarity — a fixed, absolute bar the user can reason about. It is a subscription signal answering "what new content matches something I searched for?" — compared against recent-search query embeddings only, never arbitrary content. Evaluation is strictly forward-looking: new content is evaluated against existing Recent Searches, never existing content against a new search — a video ingested Monday and a query first searched Tuesday never meet in any Digest (backward "you haven't seen this" retro-scans are the rolling-window semantics declared MVP+ on the Digest term; a Monday video that matches Tuesday's query is found by ordinary search, which is what ordinary search is for). Clearing search history removes the subscription for future runs. Evaluation runs for every run type — scheduled, manual, and Backfill — because a backfilled video matching an old query is still a subscription hit. The Digest stores the complete match list (video, triggering Recent Search, absolute percentage); renderings cap it: the dashboard shows the top 10 by absolute similarity with a view-all expansion, the Matrix excerpt the top 5. High-Signal shares fingerprints with Related Items but deliberately not their scale: Related Item percentages are rank-relative within a candidate set, High-Signal percentages are absolute, and the two are not comparable.
...
**Reprocess**:
Re-execute the full pipeline for an entity whose pipeline completed (any ingestion status other than Core-Stage `failed`, including `processed_with_warnings` — stage-level Retry Budget exhaustion never blocks it), explicitly bypassing the idempotency/skip guard. Reprocessing marks the entity's Search Documents stale by derivation, so embeddings regenerate as a downstream consequence — the user never requests embedding regeneration separately except in the bulk model-change flow. Reprocess also re-evaluates scrape-exclusion policy against the live site (an exclusion is a policy outcome, not a failure, so Retry correctly leaves it alone while the world may have changed). This is the second of the only two user-facing re-run verbs.
_Avoid_: re-ingest, refresh, redo (for succeeded work)

### Channel & dependency health

**Degraded (Channel)**:
A stored channel-level state entered after two consecutive runs fail at the adapter stage. A Degraded channel is excluded from a scheduled run's channel selection, but each scheduled run probes Degraded non-Paused channels once (a single lightweight metadata fetch) after selection: a failed probe leaves Degraded in place and increments the failure count; a successful probe clears Degraded and resets the failure counter to zero. Channels healed this way are processed in the same run's rejoin sweep (see Ingestion Run) — they never wait for the next day. Degraded is orthogonal to Deferment: an active Deferment pauses the failure counter (failures during a deferment don't count — the channel never had a fair chance), and Deferments gate rejoin-sweep work like any host-scoped work, so a channel can heal and still have its items deferred. The user can manually clear Degraded, which only resets the counter — the next run re-trips it if the problem persists. A genuinely dead channel (deleted on YouTube) is permanently Degraded: a valid terminal state whose inbox item names its exit actions ("Pause to stop probing, or delete to remove history"), because the user is the resolution mechanism — the system never auto-pauses a data source.
_Avoid_: broken channel, unhealthy channel, circuit-broken (as a separate user-facing term)

**Deferment**:
A stored, time-bound, host-scoped pause on external work (YouTube, a repository host, DeepWiki, a website host) caused by a rate-limit response. Workers check active Deferments before starting host-scoped work; work resumes after `Retry-After` or a configured default delay. A start gated by an active Deferment never reaches the host and is not an attempt: the item returns to `deferred` with `deferred_until` set from the Deferment's `retry_after_at`, consuming no Retry Budget (automatic or manual) and no Degraded failure count — the "no fair chance" principle applies uniformly from channels down to item-stages. Deferments are surfaced on the dashboard, run details, and daily digest, and can be manually cleared by the operator.
_Avoid_: rate-limit state, cooldown, backoff

### Work tracking

**Ingestion Run**:
An immutable historical record of one scheduled, manual, or backfill sweep: counts, stage summary, and final status as of `completed_at`. Once completed it is never rewritten — it answers "what happened in that sweep." Run-detail pages derive a live rollup from the current state of its items rather than re-reading the frozen record. A scheduled run has two phases within the one immutable record: the main sweep processes the channel set fixed at selection time (Active channels; Degraded channels are probed only), and the rejoin sweep — after the main sweep finishes but before `completed_at` is set — fully processes every channel whose probe healed it this run (metadata refresh, video selection in the run's window, ordinary ingestion items). Summary counters mean "work done": `channels_checked` counts channels that entered video selection (Active plus healed-and-rejoined), `channels_healed` counts probes that cleared Degraded, and failed probes appear as `channels_still_degraded`; Paused channels appear in no counter. The run detail's channel list gives the full per-channel truth (active / healed / probe failed, still degraded / paused), so the headline counts stay trend-meaningful and every configured channel is one click from an explanation — e.g. "1 channel checked, 1 healed, 1 probe failed (still degraded), 1 paused." A healed channel that fails again in the rejoin sweep counts that as failure #1 toward re-Degrading — the probe proved reachability, so one bad sweep is not a dead channel. Immutability is unaffected: it begins only at `completed_at`.
...
**Link**:
A single occurrence of a URL found in a specific video's description, pinned comment, or other source, with its position and context. Links are what extraction counts and what run-detail pages list; a Link whose canonical target is still being resolved shows a "resolving" badge. Rule of thumb: anything the user might legitimately see twice (once per video) is a Link. Links are a current-state projection synced by extraction: a Reprocess upserts by the `(video, source type, normalized URL)` identity — new URLs insert, surviving Links update position/source text/final URL in place, and Links absent from the new extraction are hard-deleted. The sync is scoped per source type: a source that fails to fetch on the Reprocess pass is excluded from that pass's sync scope, so a flaky best-effort source (the pinned comment — fetch failure is warning-only) never deletes its existing Links; only a *successful* fetch showing the Link absent deletes it. Comment authorship is not modeled: the pin itself is the creator's endorsement signal, and the comment's author is noise in MVP. Deletion is safe because Links are thin join rows carrying no user data: Overrides and Notes live on the Resource (untouched — only the video's pointer to it disappears), and retry history lives on Ingestion Items. The Reprocess domain event counts added/removed Links so the sync is visible in run detail, and `/api/videos/{id}/links` always answers "what does the description say now." Normal daily ingestion never re-syncs (the idempotency guard skips processed videos), so Link churn happens only on explicit user Reprocess.
_Avoid_: extracted URL, URL occurrence

**Resource**:
The canonical, deduplicated external thing one or more Links point at, after tracking-parameter removal and safe redirect resolution. Classification, scraping, repository association, and per-resource pages all operate on Resources. Digest counts ("2 new repositories, 3 new websites") count new Resources; run-detail extraction counts count new Links — the numbers may legitimately differ. Like Videos, Resources are never organically re-scraped in MVP — a stored successful scrape rots silently (a lapsed domain now serving a parked page keeps its archived content searchable) until a user Reprocess discovers the change. Resource Reprocess replaces stored scrape content with the fresh fetch unconditionally: there is no parked-page/quality heuristic in MVP (false-positive farm — the user asked for current state, and Overrides plus staleness handle the consequences honestly). A definitive-dead response on Reprocess — DNS NXDOMAIN, or connection refused after retries — sets the scrape to terminal `unavailable`: the last good content stays searchable, outbound links become best-effort, metadata retries stop. A parked page returning HTTP 200 is not dead — it is a live site with junk content; the user can Override or remove the Link if they care.
...
**Author**:
The human-readable attribution text stored on a video (`author_original`/`author_override`), snapshotted at ingestion from the platform and defaulting to the channel's effective name when the platform reports nothing distinct. Display-only metadata: included in the video-metadata Search Document so searching a guest or collab name can find the video, editable via override, but never an identity — Channel is the only organizational identity, and Author plays no role in filtering or relationships. The snapshot never auto-refreshes: a channel rename does not rewrite historical Author text (it is the archival record, and collabs/guests legitimately differ from the channel). What does propagate is the channel's *current* Effective Value name, included in every video-metadata Search Document at build time — so a channel rename changes the source content of exactly that class of documents, they become stale by derivation, and the next regeneration pass re-embeds the new name without touching any historical Author value.
_Avoid_: creator, uploader, owner

**Digest**:
A stored, run-scoped artifact assembled once when an Ingestion Run completes: new videos, new Resources (repositories, websites), High-Signal Matches, failed/skipped items, and active Deferments, linked to its run. "New" always means "first seen by ingestion selection" — a video or Resource that reaches success later via Retry or Reprocess is never counted as new. Instead each Digest carries a distinct recovered-since-last-run section, derived at assembly time from domain events: items that transitioned to success via Retry/Reprocess after the previous run completed (the retried Core-Stage failure that finally landed, the retried repo README that fetched). This keeps selection semantics clean, keeps Digests stored and immutable, gives the Matrix excerpt a natural line ("1 previously-failed video recovered"), and is empty on most runs — zero noise. The dashboard renders the most recent Digest (with a hint when a newer run is in progress); the Matrix notification is an excerpt of the same stored Digest — one assembly, two renderings, never independently computed. One narrow exception: the dashboard's Digest section re-derives the Deferments subsection from live state at render time (their truth decays by the hour and demands action now), while Matrix keeps the stored snapshot; all other fields stay stored everywhere. Rolling windows and "since you last looked" views are MVP+.
...
**Correction**:
An Override applied specifically to a classification field — not a separate user action. Its side effect is appending a learning example to the classifier's few-shot/rule source. Retracting the classification Override deactivates the corresponding example, since the judgment it was based on has been withdrawn. Corrections are never edited directly, only created or retracted through classification Overrides. Corrections carry a scope (`exact_url`, `domain`, `pattern`) and feed classification with exact-over-general precedence: `exact_url` Correction > rule-based classification > `domain`/`pattern` Correction > LLM classification — a domain Correction that a sponsor domain is all ads beats the LLM for any page on that domain, but can never beat the rule-based repo detector for a clean `/owner/repo` URL. Scope is constrained by classification coherence: `domain`/`pattern` scopes are offered only for classifications where whole-domain uniformity is meaningful (`ad_sponsor`, `affiliate`, `social`, `newsletter`, `course`, `merch`); `code_repository` and `website_resource` Corrections are `exact_url`-only in MVP, because multi-purpose hosts (github.com hosts repos, org pages, profiles, gists) are never uniformly classifiable — the poisoning scenario is unrepresentable rather than discouraged, and the UI scope selector enforces it.
_Avoid_: training edit, classifier feedback, reclassification (as a user verb)

**Notification**:
The single user-visible record of an outbound message attempt (e.g. a Matrix Digest send): provider, target, status, rendered body, provider message ID, attempt count, and error summary. Status is the record of truth; Retry always targets the Notification. The rendered body is stored at the first send attempt, and Retry re-sends those stored bytes — it never re-renders: the audit row's payload is unambiguous across attempts, an app upgrade mid-retry-window can't silently change message content, and the run summary a Notification announces stays the historical record of that completed run (its timestamp says so). The cost — a late retry may carry a since-expired Deferment line — is accepted; live Deferment truth stays dashboard-only per the Digest's narrow exception. Visible in ingestion-run details and admin UI.
...
**Readiness**:
The standing health surface backed by structured checks (embedding model, LLM, Whisper, Matrix, observability, backup path), re-verified on demand, on a daily schedule, and at startup. Readiness checks verify *service capability* ("can we embed with the configured model?"), never *coverage state* ("how much of the corpus is current-model?") — the three status surfaces have distinct jobs: Readiness = capability, Embedding Transition banner = coverage, Pending-Action Inbox = actionable debt. So flipping the Active Embedding Model does not invalidate `embedding_model_verified`, and a daily sweep passing mid-transition is correct, not contradictory: green readiness plus a coverage-rebuilding banner means "all systems work; corpus conversion in progress." A previously-passing check that now fails surfaces as a pending-action inbox item and dashboard warning — never as Onboarding. Full Readiness is a moment-in-time property ("all green right now"), not a gate.
...
The declared state between flipping the Active Embedding Model and completion of the bulk regeneration Operation. During transition: new ingestion and queries embed with the new model; vector search covers only new-model embeddings while text search is unaffected; the UI shows a "search coverage rebuilding" banner with progress; High-Signal Match evaluation is skipped for runs completed mid-transition. Recent-search query embeddings regenerate *first* in the bulk reprocess, ahead of content embeddings: they are few and cheap, and they are the yardstick for the transition's closing act — the catch-up run's High-Signal backfill must not compare new-model fingerprints against yardsticks that are still old-model (a silently-dead subscription surface that reports "no matches" for the wrong reason). Transition coverage therefore counts query embeddings too: the transition does not complete, and the catch-up run does not fire, while any regenerable recent-search embedding remains old-model. Scheduled ingestion runs pause during a transition (manual runs may proceed), and one catch-up Scheduled run fires immediately on completion — its Digest backfills High-Signal evaluation for videos ingested mid-transition. The transition completes when the bulk regeneration Operation completes, even if individual embeddings permanently failed (those degrade to ordinary stale-embedding inbox items, and coverage percent counts only processable documents). The state is derived — active model differs from the model of the completed embedding generation — not stored separately.
_Avoid_: reindexing, migration mode, dual-model window

**Screenshot**:
A best-effort WebP visual artifact, one per Segment, stored on a mounted volume with metadata in the database. Never load-bearing: the serving endpoint returns a placeholder instead of a 404 when the file is missing, so cards keep a stable layout. The placeholder is visually distinct from both real screenshots and broken images — a branded "screenshot pending/failed, retry available" treatment with the segment title and timestamp — and result cards prefer the platform thumbnail or no image over rendering a wall of placeholders. A DB row whose file is missing at serve time is marked failed (retryable, since screenshots are an Enrichment Stage) and logged as a domain event, but never affects Video Health beyond the enrichment warning it already carries. A video's rolled-up screenshot status is `unknown` / `pending` / `partial` (at least one segment missing its shot) / `succeeded` / `failed`.
_Avoid_: thumbnail (that's the platform-provided image), frame grab, preview

**Temp Media**:
Stage-scoped scratch files (audio/video downloads for Whisper fallback) living under a `temp/` folder with content-addressed names (`{runId}/{videoId}/{stage}-{attempt}-{contentHashPrefix}.{ext}`). Re-running a download for identical content reuses the existing file; transcription results commit to the database before Temp Media is deleted, so a mid-stage crash is harmless. Quota is enforced before download starts — exceeding it defers the item rather than failing mid-write. The effective quota is `min(ingestion.tempMedia.maxBytes, 50% of current free bytes at check time)` with a non-configurable hard floor: a download never starts if it would leave less than 1 GB free — the seeded value is the user ceiling and policy default, the live check keeps the system honest as the disk fills with Postgres, Screenshots, and debug captures. A quota-deferred item defers to the next Ingestion Run (re-checked at selection; no manual clear, no Deferment row — host-local resource pressure is a different beast from a rate limit) and surfaces as a storage warning in the Pending-Action Inbox. Anything surviving past the next startup cleanup is definitionally orphaned and removed.
...
**Interaction Boost**:
The ranking lift a Video Cluster gets from the user's open/click signals: `min(0.05, 0.01 × event count)` where events are any interaction type (result, timestamp, repository, website, or note opened) within a rolling window (default 90 days, configurable via `search.interactionBoostWindowDays`), counted per cluster and capped at five. No decay curve inside the window — it is a current-interest signal, not a fossil record. Interaction events are an audit stream, retained indefinitely: only the trailing window feeds ranking, and the boost counts per video (cluster), so document-level churn never distorts it — a deleted Note's Search Document or a re-segmented generation's old segment documents don't reduce the boost, since counting keys off the video. Consequently an event's `search_document_id` is a diagnostic reference only, never an integrity constraint: audit rows must not block hard deletion of derived artifacts, and ranking never joins events back to live documents. Events referencing `recent_search_id` / `video_id` keep their references nullable so clear-all and video deletion never destroy the audit trail either.
...
**Transcript cutover**:
The activation of a higher-preference Transcript: cue-level Search Documents are marked stale and rebuilt from the new cues, and Segments re-map to the new cues by timestamp overlap within the same Segment Generation (boundaries don't change, so no new generation). The overlap rule is any-intersection: a new cue belongs to a segment's mapping iff its `[start, end)` window intersects the segment's `[start, end)` window at all — simple, deterministic, tolerant of boundary jitter, and a cue straddling a segment boundary correctly lands in both segments' retrieval context. A segment overlapping zero new cues (timing gaps, music, late-starting captions) gets an empty mapping and no transcript-chunk documents — the honest state for a window with no transcript text; the cutover's domain event counts these segments so run detail shows e.g. "2 segments have no cues in the new transcript." Segment title/summary documents are unaffected — they come from the generation, not the cues. Cue Overrides on the old Transcript stay attached to it — preserved but inert, with no carry-forward to the new cues (a mis-applied override is worse than a lost one); the cutover records a domain event noting how many overrides became inert so the discard is visible in run detail and video events.
_Avoid_: transcript replacement, re-transcription

**Video**:
A long-form video ingested from a configured Channel — the central entity that Segments, Transcripts, Links, Screenshots, Notes, and Search Documents attach to. Video rows are created only by ingestion, never by link extraction: a YouTube URL found in a description is a plain Resource, and video-to-video linking is MVP+. An ingested Video whose platform source is later deleted keeps all its data unchanged; a Reprocess that gets a definitive "unavailable" response marks it terminally `unavailable` (metadata retries stop, watch links become best-effort) with no Video Health penalty beyond that flag. Scheduled runs never re-probe processed videos (the idempotency guard), so platform deletions are never noticed organically — silent link rot is accepted in MVP; the probe budget belongs to Channels (Degraded probes), not videos. A user Reprocess is therefore the only path that discovers unavailability, and Reprocess stays enabled on `unavailable` videos as the resurrection path: re-uploads and restores under the same video ID happen, and a successful Reprocess applies normal pipeline semantics. A Reprocess that gets "unavailable" again is not a failure — it spends no Retry Budget and simply leaves the state in place. Video detail copy: "Source unavailable on YouTube — stored data preserved. Reprocess to re-check."
...
**Pre-Corpus Search Block**:
The guard that keeps the flagship feature's first impression from being a void: until the first Video row exists, the search page redirects to a waiting state ("your library is being built — first run at the scheduled time") with a run-now action, and search is not yet a selectable mode. The block lifts the moment any Video row exists — run completion is not required; a mid-run corpus is searchable, and the search page shows a "library still building" banner with live ingestion progress until the first run completes. A completed run with zero ingested videos leaves the block in place, with copy branched on run type: a scheduled or manual zero-video run shows backfill guidance; a zero-video Backfill shows channel-level guidance ("this channel may have no long-form videos in the window — check the channel or widen bounds"), since the user just tried the obvious widening move. Because searches can't happen pre-corpus, zero-result searches on an empty corpus are never stored as Recent Searches.
_Avoid_: empty state, no-results page
...
**Retry Budget**:
The bounded retry allowance for a failed item-stage: up to 2 automatic backoff attempts, then up to 5 user-triggered manual Retries. Only starts that actually reach the external host consume the budget — a start gated by an active Deferment is not an attempt: the item returns to `deferred` with `deferred_until` set from the Deferment's `retry_after_at`, its budget untouched, and resumes when the Deferment expires (automatic attempts continue their backoff schedule; manually retried items stay queued as `deferred` and visible on run detail). The budget bounds real host contact, which is what it exists for. Reaching the cap marks the item permanently failed (`is_retryable = false`) with an explanation in the UI; the only paths forward are Reprocess — a deliberate fresh start that resets the budget — or exclusion rules that skip the URL. This bounds host hammering and keeps the inbox from becoming a retry slot machine.
_Avoid_: retry limit, attempt cap, max retries

**Recall Harness**:
The regression gate for the killer journey: a golden dataset of at least 20 vague natural-language queries, each written query-first — before its fixture video's metadata is finalized — so queries can't be reverse-engineered from the text. The gate runs against a corpus padded to the MVP scale assumption (~500 videos): golden fixture videos are the recall targets and synthetic, topically-adjacent distractors fill the rest, so "top 3" means top 3 of ~500. The gate is 100% top-3 recall on the dataset, no partial pass; any regression is fixed in ranking, weights, or document construction, never by editing the dataset to fit. The dataset grows via a "this should have ranked" affordance in search: a failed real query is captured with its expected video, score components, and formula version into a review queue, and promotion into the dataset is a deliberate act. It runs on every ranking-formula, Embedding Model, or document-construction change. It is a regression floor — "queries like these stay top-3" — not a claim about all possible vague queries.
