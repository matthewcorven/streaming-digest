...
public sealed class PostgresMigrationBaselineTests : IAsyncLifetime
{
    ...
    private readonly string _containerName = $"streaming-digest-tests-{Guid.NewGuid():N}";
    ...
    [Fact]
    public async Task Initial_baseline_applies_to_postgres()
    {
        ...
        await using var command = new NpgsqlCommand("SELECT to_regclass('public.app_users') IS NOT NULL, to_regclass('public.channels') IS NOT NULL, to_regclass('public.videos') IS NOT NULL, to_regclass('public.domain_events') IS NOT NULL, to_regclass('public.digests') IS NOT NULL", connection);
