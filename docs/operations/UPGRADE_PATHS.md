...
| DB app setting addition | Add or recalibrate `search.highSignalThresholdPercent` | No | Seed missing keys; one-time startup migration may rewrite legacy seeded defaults | Preserve non-legacy user value if already present |
